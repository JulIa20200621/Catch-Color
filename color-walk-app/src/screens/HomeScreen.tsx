      ...
      <View style={styles.page}>
        <View style={styles.topLine}>
          <View style={styles.colorInfo}>
            ...
            <Pressable
              ...
              onPress={() => setDailyTarget(createPersonalTarget(target))}
            >
              <Ionicons name="refresh" size={12} color={colors.primary} />
              <Text style={styles.changeText}>换一色</Text>
            </Pressable>
          </View>
          <Text style={styles.counter}>
            {mode === "multi" ? "已收集：0 色" : "已拍：0 张"}
          </Text>
        </View>
        ...
        <View style={[styles.viewfinder, { backgroundColor: color.soft }]}>
          ...
          <View style={styles.meta}>
            <Text style={styles.metaText}>iso 100 / f 1.8</Text>
          ...
          <View style={styles.zoomPill}>
            {["0.5x", "1x", "2x"].map((label, index) => (
              <Pressable key={label} onPress={() => setZoom(index)}>
                <Text
                  style={[styles.zoomText, zoom === index && styles.zoomActive]}
                >
                  {label}
                </Text>
              </Pressable>
        ...
        <View style={styles.shutterRow}>
          <Pressable
            style={[
              styles.sideButton,
              {
                ...
                backgroundColor: flash ? color.hex : color.soft,
              },
            ]}
          ...
          >
            <Ionicons
              name="flash-outline"
              size={18}
              color={flash ? colors.white : colors.ink}
            />
          </Pressable>
          <Pressable
            accessibilityLabel="使用后置摄像头拍摄"
            style={[styles.shutter, { borderColor: color.hex }]}
            onPress={() =>
              navigation.navigate("Camera", { initialFacing: "back" })
            }
          >
            <View
              style={[styles.shutterInner, { backgroundColor: color.hex }]}
            />
          </Pressable>
          <Pressable
            accessibilityLabel="使用前置摄像头拍摄"
            style={[
              styles.sideButton,
              { borderColor: color.hex, backgroundColor: color.soft },
            ]}
            onPress={() =>
              navigation.navigate("Camera", { initialFacing: "front" })
            }
          >
            <Ionicons
        ...
        {mode === null ? (
          <Pressable style={styles.mask} onPress={() => setMode(null)}>
            ...
            >
              ...
              >
                <Text style={styles.defaultActionText}>🎨 一个颜色拍多张</Text>
              </Pressable>
              <Pressable
                style={styles.primaryAction}
                onPress={() => setMode("multi")}
              >
                <Text style={styles.primaryActionText}>🧺 收集不同颜色</Text>
              </Pressable>
            </Pressable>
          </Pressable>
        ) : null}
...
const styles = StyleSheet.create({
  ...
  mask: {
    ...
    backgroundColor: "rgba(59,59,62,0.25)",
  },
  modeCard: {
    width: "100%",
    maxWidth: 320,
