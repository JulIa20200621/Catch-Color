...
function photoColorId(photo: PhotoRecord): string | null {
  ...
  return (
    app2Colors.find((color) => color.hex.toLowerCase() === targetHex)?.id ??
    null
  );
}
...
export function AlbumScreen({ navigation }: Props) {
  ...
  const photosOfColor = useMemo(
    () =>
      colorFilter
        ? ownedPhotos.filter((photo) => photoColorId(photo) === colorFilter)
        : [],
  ...
  return (
    <Screen>
      <View style={styles.page}>
        ...
        >
          {tab === "color" ? (
            <View style={styles.animalGrid}>
              {app2Animals.map(([id, animalName, emoji], index) => {
                const color = app2Colors[index];
                const count = ownedPhotos.filter(
                  (photo) => photoColorId(photo) === color.id,
                ).length;
                ...
                return (
                  ...
                  >
                    ...
                    <Text style={styles.animalColor}>{color.name}</Text>
                ...
                {ownedPhotos.map((photo) => (
                  ...
                  >
                    ...
                    <Text style={styles.galleryDate}>
                      {dateLabel(photo.createdAt).slice(5)}
                    </Text>
                  </Pressable>
                ))}
              </View>
          ...
          {tab === "map" ? (
            <View style={styles.map}>
              ...
              <Text style={styles.mapTitle}>色彩漫步地图</Text>
              <Text style={styles.mapText}>
                已记录 {ownedPhotos.filter((photo) => photo.location).length}{" "}
                个有定位的色彩足迹
              </Text>
              <Pressable
              ...
              >
                <Text style={styles.mapActionText}>查看我的真实足迹</Text>
              </Pressable>
            </View>
          ) : null}
        </ScrollView>
        <Modal
          visible={colorFilter !== null}
          transparent
          animationType="slide"
          onRequestClose={() => setColorFilter(null)}
        >
          <Pressable style={styles.modalMask} onPress={() => setColorFilter(null)}>
            <Pressable style={styles.drawer} onPress={(event) => event.stopPropagation()}>
              <View style={styles.drawerHandle} />
              <Text style={styles.drawerTitle}>
                {colorFilter
                  ? `${getApp2Color(colorFilter).name} · 色彩小书`
                  : ""}
              </Text>
              {photosOfColor.length ? (
                <ScrollView contentContainerStyle={styles.bookGrid}>
                  {photosOfColor.map((photo) => (
                    <Pressable
                      key={photo.id}
                    ...
                    >
                      ...
                      <Text style={styles.bookName}>
                        {photo.target.colorName}
                      </Text>
                      <Text style={styles.bookDate}>
                        {dateLabel(photo.createdAt)}
                      </Text>
                    </Pressable>
                  ))}
                </ScrollView>
              ) : (
                <Text style={styles.emptyBook}>还没有拍到这个颜色的照片。</Text>
              )}
            </Pressable>
          </Pressable>
        </Modal>
        ...
        >
          <Pressable style={styles.detailMask} onPress={() => setSelected(null)}>
            ...
            {selected ? (
              <Pressable style={styles.detailCard} onPress={(event) => event.stopPropagation()}>
                ...
                <Text style={styles.detailName}>
                  {selected.target.colorName}
                </Text>
                <Text style={styles.detailMeta}>
                  {dateLabel(selected.createdAt)}
                </Text>
                <Text style={styles.detailLocation}>
                  {selected.location ? "已记录拍摄位置" : "未记录拍摄位置"}
                </Text>
              </Pressable>
            ) : null}
          </Pressable>
...
const styles = StyleSheet.create({
  ...
  animalGrid: { flexDirection: "row", flexWrap: "wrap", rowGap: 20 },
  animalItem: { width: "33.333%", alignItems: "center" },
  animalCircle: {
    width: 80,
    height: 80,
    borderRadius: 26,
  ...
  animalImage: { width: 68, height: 68 },
  ...
  mapActionText: { color: colors.white, fontSize: 13, fontWeight: "700" },
  modalMask: {
    flex: 1,
    justifyContent: "flex-end",
    backgroundColor: "rgba(59,59,62,0.28)",
  },
  drawer: {
    minHeight: "65%",
    maxHeight: "78%",
  ...
  bookGrid: { flexDirection: "row", flexWrap: "wrap", rowGap: 20 },
  bookItem: { width: "33.333%", paddingHorizontal: 5 },
  bookArt: { width: "100%", aspectRatio: 0.75, borderRadius: 6 },
  bookName: {
    marginTop: 8,
  ...
  bookDate: { marginTop: 2, color: colors.inkMuted, fontSize: 10 },
  emptyBook: {
    paddingVertical: 36,
    color: colors.inkMuted,
    textAlign: "center",
  ...
  detailMask: {
    ...
    justifyContent: "center",
    padding: 28,
    backgroundColor: "rgba(59,59,62,0.86)",
  },
  close: {
    position: "absolute",
    top: 52,
  ...
  closeText: { color: colors.white, fontSize: 32, fontWeight: "300" },
  detailCard: { padding: 14, borderRadius: 6, backgroundColor: colors.white },
  detailArt: { width: "100%", aspectRatio: 0.82, borderRadius: 6 },
  detailName: {
    marginTop: 16,
    color: colors.primary,
    fontSize: 19,
  ...
  detailLocation: { marginTop: 5, color: colors.ink, fontSize: 11 },
});
