...
import { createCommunityComment, createCommunityPost, loadCommunityComments, loadCommunityPosts, loadFriends, recordCommunityShare, requestFriend, toggleCommunityLike, type CommunityCommentRecord } from '../services/supabaseData';
...
export function CommunityScreen({ navigation }: Props) {
  ...
  const saveFriend = async () => {
    ...
    try {
      ...
      Alert.alert('好友申请已发送', '对方确认后会出现在你的伙伴列表中。');
    } catch (error) {
  ...
  const openComments = async (post: FeedPost) => {
    ...
    try {
      setComments(await loadCommunityComments(post.id));
    } catch (error) {
      setComments([]);
      Alert.alert('读取评论失败', error instanceof Error ? error.message : '请稍后重试');
    } finally {
  ...
  const submitComment = async () => {
    if (!commentingPost || !account?.id) return;
    const body = commentText.trim();
    if (!body) return;
    setCommentSubmitting(true);
    try {
      await createCommunityComment(account.id, commentingPost.id, body);
      setCommentText('');
      setComments(await loadCommunityComments(commentingPost.id));
    } catch (error) {
      Alert.alert('发布评论失败', error instanceof Error ? error.message : '请稍后重试');
    } finally {
  ...
  <Modal visible={publishing} transparent animationType="slide" onRequestClose={() => setPublishing(false)}>
    <Pressable style={styles.mask} onPress={() => setPublishing(false)}>
      <Pressable style={styles.drawer} onPress={(event) => event.stopPropagation()}>
        <View style={styles.handle} />
        <Text style={styles.drawerTitle}>发布色彩动态</Text>
        <Text style={styles.fieldLabel}>选择自己的照片</Text>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.photoPicker}>
          {photos.map((photo: PhotoRecord) => <Pressable key={photo.id} style={[styles.pickTile, selectedPhotoId === photo.id && styles.pickTileActive]} onPress={() => setSelectedPhotoId(photo.id)}><Image source={{ uri: photo.imageUri }} style={styles.pickImage} /><View style={[styles.pickColor, { backgroundColor: photo.target.colorHex }]} /></Pressable>)}
        </ScrollView>
        <Text style={styles.fieldLabel}>想说的话</Text>
        <TextInput value={caption} onChangeText={setCaption} maxLength={140} multiline placeholder="比如：散步时遇到一块特别温柔的蓝。" placeholderTextColor="#B4A88E" style={styles.captionInput} />
        <Text style={styles.syncHint}>发布后将立即写入社区；原图仍保留在私有相册 bucket 中。</Text>
        <Pressable style={styles.publishSubmit} onPress={() => void publish()}><Text style={styles.publishSubmitText}>发布动态</Text></Pressable>
      </Pressable>
    </Pressable>
  </Modal>
  <Modal visible={addingFriend} transparent animationType="slide" onRequestClose={() => setAddingFriend(false)}>
    <Pressable style={styles.mask} onPress={() => setAddingFriend(false)}>
      <Pressable style={styles.friendDrawer} onPress={(event) => event.stopPropagation()}>
        <View style={styles.handle} />
        <Text style={styles.drawerTitle}>添加伙伴</Text>
        <Text style={styles.fieldLabel}>对方的公开用户 ID</Text>
        <TextInput value={friendLookup} onChangeText={setFriendLookup} autoCapitalize="none" autoCorrect={false} placeholder="例如：colorwalker" placeholderTextColor="#B4A88E" style={styles.singleInput} />
        <Text style={styles.fieldLabel}>备注昵称（可选）</Text>
        <TextInput value={friendNickname} onChangeText={setFriendNickname} maxLength={12} placeholder="例如：小满" placeholderTextColor="#B4A88E" style={styles.singleInput} />
        <Text style={styles.syncHint}>会向对应用户发送真实好友申请；对方确认后即可成为伙伴。</Text>
        <Pressable disabled={friendSubmitting} style={[styles.publishSubmit, friendSubmitting && { opacity: 0.55 }]} onPress={() => void saveFriend()}><Text style={styles.publishSubmitText}>{friendSubmitting ? '正在发送…' : '发送好友申请'}</Text></Pressable>
      </Pressable>
    </Pressable>
  </Modal>
  <Modal visible={commentingPost !== null} transparent animationType="slide" onRequestClose={() => setCommentingPost(null)}>
    <Pressable style={styles.mask} onPress={() => setCommentingPost(null)}>
      <Pressable style={styles.commentDrawer} onPress={(event) => event.stopPropagation()}>
        <View style={styles.handle} />
        <Text style={styles.drawerTitle}>评论</Text>
        {commentLoading ? <ActivityIndicator style={styles.commentLoader} color={colors.coral} /> : null}
        {!commentLoading && !comments.length ? <Text style={styles.emptyComment}>还没有评论，留下第一句吧。</Text> : null}
        <ScrollView style={styles.commentList} contentContainerStyle={styles.commentListContent} keyboardShouldPersistTaps="handled">
          {comments.map((comment) => <View key={comment.id} style={styles.commentRow}><Text style={styles.commentName}>{comment.nickname}</Text><Text style={styles.commentBody}>{comment.body}</Text></View>)}
        </ScrollView>
        <View style={styles.commentComposer}>
          <TextInput value={commentText} onChangeText={setCommentText} maxLength={240} placeholder="写下你的想法…" placeholderTextColor="#B4A88E" style={styles.commentInput} />
          <Pressable disabled={!commentText.trim() || commentSubmitting} style={[styles.commentSubmit, (!commentText.trim() || commentSubmitting) && { opacity: 0.5 }]} onPress={() => void submitComment()}><Text style={styles.commentSubmitText}>{commentSubmitting ? '发送中' : '发送'}</Text></Pressable>
        </View>
      </Pressable>
    </Pressable>
  </Modal>
